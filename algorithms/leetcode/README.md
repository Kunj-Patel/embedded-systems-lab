# Algorithm Interview Prep Tracking

**Current Count:** 2 / 150

## Strategy & Rules
1. **The Path:** Following the **NeetCode 150** roadmap
2. **The 15-Minute Rule:** As a beginner, do not stare at a blank screen for more than 15 minutes. If completely stuck, watch the solution, understand the pattern, code it up, and write down *why* it works. The goal is learning patterns, not taking a test.

## Completed Problems

| Date | Problem | Difficulty | Pattern | Time | Space |
|---|---|---|---|---|---|
| 14/07/2026 | [217. Contains Duplicate](01-array-easy.md) | Easy | Array / Hashing | O(n) | O(n) |
| 14/07/2026 | [242. Valid Anagram](02-string-easy.md) | Easy | String / Hashing | O(n) | O(1) |
